...
static char timing_tx_buffer[256];
...
static void App_ReportTiming(void)
{
  ...
  uint32_t jitter_peak_to_peak_us;
  int length;
  ...
  jitter_peak_to_peak_us =
      stats.period_max_us - stats.period_min_us;
  length = snprintf(
      timing_tx_buffer,
      sizeof(timing_tx_buffer),
      "TIMING imu_pid rate=%lu.%03luHz samples=%lu "
      "period_us=%lu/%lu/%lu jitter_pp_us=%lu "
      "pipeline_us=%lu/%lu/%lu not_ready=%lu errors=%lu dwt=%u\r\n",
      (unsigned long)(stats.actual_rate_millihz / 1000U),
      (unsigned long)(stats.actual_rate_millihz % 1000U),
      (unsigned long)stats.sample_count,
      (unsigned long)stats.period_min_us,
      (unsigned long)stats.period_mean_us,
      (unsigned long)stats.period_max_us,
      (unsigned long)jitter_peak_to_peak_us,
