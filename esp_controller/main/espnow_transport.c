...
esp_err_t espnow_transport_start(espnow_transport_packet_callback_t callback)
{
    ...
    const uint8_t configured_peer[ESP_NOW_ETH_ALEN] =
        DRONE_ESPNOW_PEER_MAC_BYTES;
    memcpy(s_peer_mac, configured_peer, sizeof(s_peer_mac));
    ...
    ESP_RETURN_ON_ERROR(esp_wifi_set_channel(DRONE_ESPNOW_CHANNEL,
    ...
    peer.channel = DRONE_ESPNOW_CHANNEL;
...
esp_err_t espnow_transport_get_link_info(espnow_transport_link_info_t *info)
{
    ...
    info->channel = DRONE_ESPNOW_CHANNEL;
    return ESP_OK;
}
